...
async def upload_file(file: UploadFile = File(...)):
    x = await file.read()
    df = pd.read_csv(io.BytesIO(x), index_col=False, dtype='unicode')
    ...
    if count_0 > count_1:  #BENIGN
        return {"Prediction": "Not Malicious", "nonmal" : count_0, "mali" : count_1, "attack": "NA"}
        ...
        if type_of_the_attack=="":
            type_of_the_attack="Zero-day Attack"
...
def getchatgpt(attack):
    ...
    prompt = f"Give information of the {attack} attack and tips to resolve it"
